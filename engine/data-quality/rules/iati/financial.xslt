  ...
  <xsl:choose>
    <xsl:when test="receiver-org[@ref=current()/../reporting-org/@ref 
      ...
      <me:feedback type="success" class="financial" id="7.3.3">
        ...
        <me:message>The incoming transaction does not need receiver information (although the current information matches the activity).</me:message>
      </me:feedback>      
    </xsl:when>
...
<xsl:template match="transaction[transaction-type/@code=('2','3','D','C')]" mode="rules" priority="7.4">
  <xsl:choose>
    <xsl:when test="provider-org[@ref=current()/../reporting-org/@ref 
      ...
      <me:feedback type="success" class="financial" id="7.4.3">
        <me:src ref="practice" versions="all"/>
        <me:message>The outgoing transaction does not need provider information (although the current information matches the activity).</me:message>
      </me:feedback>      
    </xsl:when>
    <xsl:when test="provider-org">
      <me:feedback type="danger" class="financial" id="7.4.2">
        <me:src ref="practice" versions="all"/>
        <me:message>The outgoing transaction does not need receiver information, and the information given does not match the activity.</me:message>
      </me:feedback>
    </xsl:when>
  </xsl:choose>
  ...
  <xsl:if test="not(receiver-org/@ref!='') and not(receiver-org/narrative!='')">
    <me:feedback type="warning" class="financial" id="7.4.1">
      <me:src ref="iati-doc" versions="all"/>
