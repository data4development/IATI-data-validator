...
<xsl:template match="document-link" mode="rules" priority="6.1">
  ...
  <xsl:if test="@format=$unlikelyformats">
    <me:feedback type="info" class="information" id="6.1.1">
      ...
      <me:message>The document format is specified as <code><xsl:value-of select="@format"/></code>.
      This is an unlikely format for a document.</me:message>
    </me:feedback>
  </xsl:if>
  ...
  <xsl:if test="not(@format = $mime-types)">
    <me:feedback type="warning" class="information" id="6.1.2">
      ...
      <me:message>The document format <code><xsl:value-of select="@format"/></code>
        is not a known media type.</me:message>
    </me:feedback>
  </xsl:if>
...
<xsl:template match="iati-activity" mode="rules" priority="6.2">
  ...
  <xsl:if test="not(sector) and not(transaction/sector)">
    <me:feedback type="warning" class="information" id="6.2.2">
      ...
      <me:message>The activity has no sector classification information for either
      the activity or the transactions.</me:message>
    </me:feedback>
  </xsl:if>
  ...
  <xsl:if test="sector and not(sector/@vocabulary='1' or not(sector/@vocabulary))">
    <me:feedback type="warning" class="information" id="6.2.2">
      ...
      <me:message>The activity has no sector classification information for the OECD DAC
      sector vocabulary.</me:message>
    </me:feedback>
  </xsl:if>
