...
<xsl:stylesheet version='3.0' xmlns:xsl='http://www.w3.org/1999/XSL/Transform'
  ...
  <xsl:template match="indicator" mode="rules" priority="8.1">
    ...
    <xsl:if test="not(period)">
      <me:feedback type="warning" class="results" id="8.1.2">
        <me:src ref="practice" versions="any"/>
  ...
  <xsl:template match="result" mode="rules" priority="8.2">
    ...
    <xsl:if test="not(@aggregation-status)">
      <me:feedback type="warning" class="results" id="8.2.1">
  ...
  <xsl:template match="indicator/reference" mode="rules" priority="8.3">
    ...
    <xsl:if test="@vocabulary='99' and (not(@indicator-uri) or @indicator-uri='')">
      <me:feedback type="info" class="results" id="8.3.1">
        <me:src ref="practice" versions="any"/>
  ...
  <xsl:template match="baseline" mode="rules" priority="8.4">
    ...
    <xsl:choose>
      ...
      <xsl:when test="xs:integer(@year) lt $start-year - 5">
        <me:feedback type="warning" class="results" id="8.4.2">
          <me:src ref="practice" versions="any"/>
      ...
      <xsl:when test="xs:integer(@year) gt $end-year">
        <me:feedback type="warning" class="results" id="8.4.3">
          <me:src ref="practice" versions="any"/>
  ...
  <xsl:template match="baseline/@value|target/@value|actual/@value" mode="rules" priority="8.5">
    <xsl:if test="ancestor::result/@aggregation-status and not(. castable as xs:decimal)">
      <me:feedback type="danger" class="results" id="8.5.1">
        <me:src ref="iati" versions="any"/>
  ...
  <xsl:template match="period" mode="rules" priority="8.6">
    ...
    <xsl:if test="period-start/@iso-date gt period-end/@iso-date">
      <me:feedback type="danger" class="results" id="8.6.1">
        <me:src ref="iati" versions="any"/>
    ...
    <xsl:if test="not(target)">
      <me:feedback type="warning" class="results" id="8.6.2">
        <me:src ref="iati" versions="any"/>
  ...
  <xsl:template match="period/*[self::period or self::target]/location" mode="rules" priority="8.7">
    ...
    <xsl:if test="not(@ref=ancestor::iati-activity/location/@ref)">
      <me:feedback type="danger" class="results" id="8.7.1">
        <me:src ref="iati" versions="any"/>
  ...
  <xsl:template match="dimension" mode="rules" priority="8.8">
    ...
    <xsl:if test="not(@name) or @name=''">
      <me:feedback type="error" class="results" id="8.8.1">
        <me:src ref="iati" versions="any"/>
        <me:message>The location reference is not specified in this activity.</me:message>
      </me:feedback>
    </xsl:if>
