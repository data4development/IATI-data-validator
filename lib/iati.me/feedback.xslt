...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="budget/me:feedback" mode="context"> In the budget of <xsl:value-of
  ...
  <xsl:template match="provider-org/me:feedback | receiver-org/me:feedback" mode="context"> In
  ...
  <!-- Context information for participating-org -->
  <xsl:template match="document-link/me:feedback" mode="context"> For the document <a
      href="{../@url}"><xsl:apply-templates select="../title"/></a>: </xsl:template>

  <xsl:template match="result/me:feedback | indicator/me:feedback" mode="context"> For the
    {name(..)} "<em><xsl:apply-templates select="../title"/></em>": </xsl:template>

  <xsl:template
    match="baseline/me:feedback | indicator/reference/me:feedback | indicator/description/me:feedback | result/description/me:feedback"
    mode="context"> For the {name(../..)} "<em><xsl:apply-templates select="../../title"/></em>": </xsl:template>

  <xsl:template match="location/description/me:feedback" mode="context"> For the {name(../..)}
        "<em><xsl:apply-templates select="../../name"/></em>": </xsl:template>

  <xsl:template match="target/me:feedback | actual/me:feedback" mode="context"> For the indicator
        "<em><xsl:apply-templates select="../../../title"/></em>" in the period
    {../../period-start/@iso-date} to {../../period-end/@iso-date}: </xsl:template>

  <xsl:template match="period/me:feedback" mode="context"> In the indicator
        "<em><xsl:apply-templates select="../../title"/></em>" in the period
    {../period-start/@iso-date} to {../period-end/@iso-date}: </xsl:template>
